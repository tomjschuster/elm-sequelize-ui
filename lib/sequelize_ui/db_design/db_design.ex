defmodule SequelizeUi.DbDesign do
  ...
  defp add_primary_key(%Column{} = column, %Constraint{} = constraint) do
    with col_con_attrs <- con_coll_attrs(column.id, nil, constraint.id),
 ...
 defp constraint_attrs(%Table{} = table, enum, value \\ nil) do
  ...
  defp build_constraint(%Table{} = table, %Column{} = column, attrs) do
    with {:ok, constraint} <- create_constraint(attrs),
         col_con_attrs <- con_coll_attrs(column.id, nil, constraint.id),
         {:ok, column_constraint} <- create_column_constraint(col_con_attrs),
         do: {:ok, constraint}
  end
  ...
  defp build_foreign_keys(%Table{} = table, %Column{} = column, attrs, references) do
    with {:ok, constraint} <- create_constraint(attrs),
         all_col_con_attrs <- Enum.map(references, &con_coll_attrs(column.id, &1, constraint.id)),
         results <- Enum.map(all_col_con_attrs, &create_column_constraint/1),
         nil <- Enum.find(results, fn {status, value} -> status === :error end),
         do: {:ok, constraint}
  end

  defp con_coll_attrs(column_id, references_id, constraint_id) do
    %{
      column_id: column_id,
